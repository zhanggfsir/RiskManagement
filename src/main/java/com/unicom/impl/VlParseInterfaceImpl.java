...
public class VlParseInterfaceImpl implements ParseInterface {
    ...
    public Put getPut(String str, LoadColumnInfo loadColumnInfo, Path path, String account) {
        Risk.ValueList.Builder valueList=Risk.ValueList.newBuilder();
        ...
        String user_level=array[1].trim();
        //如果用户没有等级就跳过这个用户
        if(StringUtils.isEmpty(user_level) ) {
            return  null;
        }
        try { valueList.setUserLevel(Integer.parseInt(user_level)); } catch (NumberFormatException e) { }
        valueList.setUserLevelTime(array[2]);

        try {
        buff = valueList.build().toByteArray();} catch (Exception e) {
