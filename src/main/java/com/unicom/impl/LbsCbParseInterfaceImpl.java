 ...
 * }
 */
public class LbsCbParseInterfaceImpl implements ParseInterface {
...
//        String[] array = StringUtils.splitPreserveAllTokens(str,loadColumnInfo.getSeperator());
        ...
        String imsi=array[14];
        String mcc=StringUtils.substring(imsi,0,3);

        lBSBuild.setProvId(provId);
        lBSBuild.setAreaId(areaId);
        lBSBuild.setMcc(mcc);

        String deviceNumber=array[15];
        String startTime=array[9];

        buff = lBSBuild.build().toByteArray();
        //rowkey: device_number_md5,yyyyMMdd,hhmmss  // 主叫年月日时分秒
        StringBuilder keyValue=new StringBuilder();
        Md5Util md5Util=new Md5Util();
        String deviceNumberMd5=md5Util.md5(deviceNumber);
        keyValue.append(deviceNumberMd5);

        String yyyyMMdd=StringUtils.substring(startTime,0,8);
        String hhmmss=StringUtils.substring(startTime,8);
