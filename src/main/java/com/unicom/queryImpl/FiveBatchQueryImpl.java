...
public class FiveBatchQueryImpl implements QueryBatchInterface{
    ...
    public HashMap<String,String>  getFieldFromHdfs(String linePath, String[] arrayField,HashMap<String,String> hdfsMap,String account) throws IOException {
        ...
        sb.append("|");
        sb.append(StringUtils.isBlank(arrayField[10]) ? "" : Integer.parseInt(arrayField[10]));
        //拼接得到k
        String deviceNumberMd5 = arrayField[0].toUpperCase();
        String rn=arrayField[11];
        account=account.substring(0,6);

        String  k=deviceNumberMd5+"|"+rn+"|"+account;
    ...
    public void getDataFromHbaseAndWrite(ZkInfo zkInfo,String tableName,String familyName,String columnName,
        ...
        while (it.hasNext()) {
            ...
            String rn=StringUtils.splitPreserveAllTokens(rowkeyFromHdfsMap, "|")[1];
            ...
            account=account.substring(0,6);
            String singleRowkey = deviceNumberMd5 +rn+account;
            ...
            if (!result.isEmpty()) {
                ...
                for (String family : familyName.split(",")) {
                    ...
                    for (Cell cell : list) {
                        ...
                        Risk.ImeiChange imeiChange = Risk.ImeiChange.parseFrom(CellUtil.cloneValue(cell));
                        if(imeiChange.hasImei()) sb.append(imeiChange.getImei());
                        sb.append("|");
                        if(imeiChange.hasImsi()) sb.append(imeiChange.getImsi());
                        sb.append("|");
                        if(imeiChange.hasFactoryId()) sb.append(imeiChange.getFactoryId());
                        sb.append("|");
                        if(imeiChange.hasTermId()) sb.append(imeiChange.getTermId());
                        sb.append("|");
                        if(imeiChange.hasFactoryDesc()) sb.append(imeiChange.getFactoryDesc());
                        sb.append("|");
                        if(imeiChange.hasTermDesc()) sb.append(imeiChange.getTermDesc());
                        sb.append("|");
                        if(imeiChange.hasCityNo()) sb.append(imeiChange.getCityNo());
                        sb.append("|");
                        if(imeiChange.hasUpTime()) sb.append(imeiChange.getUpTime());
                        sb.append("|");
                        if(imeiChange.hasIsCopy()) sb.append(imeiChange.getIsCopy());
                        sb.append("|");
                        if(imeiChange.hasUseTimes()) sb.append(imeiChange.getUseTimes());
