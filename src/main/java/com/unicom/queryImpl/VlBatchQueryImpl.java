...
public class VlBatchQueryImpl implements QueryBatchInterface{
    ...
    public HashMap<String,String>  getFieldFromHdfs(String linePath, String[] arrayField,HashMap<String,String> hdfsMap,String account) throws IOException {
        ...
        String user_level=arrayField[1].trim();
        //如果用户没有等级就跳过这个用户
            ...
            if (!result.isEmpty()) {
                ...
                for (String family : familyName.split(",")) {
                    ...
                    for (Cell cell : list) {
                        ...
                        Risk.ValueList valueList = Risk.ValueList.parseFrom(CellUtil.cloneValue(cell));
                        if(valueList.hasUserLevel()) sb.append(valueList.getUserLevel());
                        sb.append("|");
                        if(valueList.hasUserLevelTime()) sb.append(valueList.getUserLevelTime());
