...
public class ScBatchQueryImpl implements QueryBatchInterface{
    ...
    public HashMap<String,String>  getFieldFromHdfs(String linePath, String[] arrayField,HashMap<String,String> hdfsMap,String account) throws IOException {
        ...
        //拼接得到k
        String deviceNo=arrayField[0];
        String  k=deviceNo+"|"+account;
    ...
    public void getDataFromHbaseAndWrite(ZkInfo zkInfo,String tableName,String familyName,String columnName,
        ...
        while (it.hasNext()) {
            ...
            String  deviceNo= StringUtils.splitPreserveAllTokens(rowkeyFromHdfsMap, "|")[0];

            //得到k  获得主键
            byte[] hash = Bytes.toBytes((short) (deviceNo.hashCode() & 0x7FFF));
            String singleRowkey = deviceNo;
            ...
            if (!result.isEmpty()) {
                ...
                for (String family : familyName.split(",")) {
                    ...
                    for (Cell cell : list) {
                        ...
                        Risk.MsisdnSeg msisdnSeg = Risk.MsisdnSeg.parseFrom(CellUtil.cloneValue(cell));
                        if(msisdnSeg.hasServType()) sb.append(msisdnSeg.getServType());
                        sb.append("|");
                        if(msisdnSeg.hasAreaCode()) sb.append(msisdnSeg.getAreaCode());
                        sb.append("|");
                        if(msisdnSeg.hasProvName()) sb.append(msisdnSeg.getProvName());
                        sb.append("|");
                        if(msisdnSeg.hasAreaName()) sb.append(msisdnSeg.getAreaName());
                        sb.append("|");
                        if(msisdnSeg.hasProvId()) sb.append(msisdnSeg.getProvId());
                        sb.append("|");
                        if(msisdnSeg.hasAreaId()) sb.append(msisdnSeg.getAreaId());
