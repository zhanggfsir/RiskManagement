...
public class imsiBatchQueryImpl implements QueryBatchInterface{
    ...
    public HashMap<String,String>  getFieldFromHdfs(String linePath, String[] arrayField,HashMap<String,String> hdfsMap,String account) throws IOException {
        ...
        //拼接得到k
        String imsi = arrayField[1];
        String  k=imsi+"|"+account;
    ...
    public void getDataFromHbaseAndWrite(ZkInfo zkInfo,String tableName,String familyName,String columnName,
        ...
        while (it.hasNext()) {
            ...
            String imsi = StringUtils.splitPreserveAllTokens(rowkeyFromHdfsMap, "|")[0];
            account= StringUtils.splitPreserveAllTokens(rowkeyFromHdfsMap, "|")[1];

            //得到k  获得主键
            byte[] hash = Bytes.toBytes((short) (imsi.hashCode() & 0x7FFF));
            String singleRowkey = imsi + account;
            ...
            if (!result.isEmpty()) {
                ...
                for (String family : familyName.split(",")) {
                    ...
                    for (Cell cell : list) {
                        ...
                        Risk.Imsi2Msisdn imsi2Msisdn = Risk.Imsi2Msisdn.parseFrom(CellUtil.cloneValue(cell));
                        if(imsi2Msisdn.hasDeviceNumberMd5()) sb.append(imsi2Msisdn.getDeviceNumberMd5());
                        sb.append("|");
                        if(imsi2Msisdn.hasUpTime()) sb.append(imsi2Msisdn.getUpTime());
