...
/**
 java -Xmx4096m -cp risk-1.0-jar-with-dependencies.jar com.unicom.queryImpl.queryIndividualization.Duxiaoman_old 419 p_duxiaoman_monthly
 q 262CA5841FCF3D2936646608F90AEE61 201909
*/

public class Duxiaoman_old {
    static Logger logger = LoggerFactory.getLogger(Duxiaoman_old.class);
...
//        CreateTableInfo createTableInfo=getConfigInfo.getConfigTableInfo(tableName);
        ...
        if ((null != deviceNumberEn) && (!deviceNumberEn.isEmpty())) {
            ...
            for (String oneKey : rowkeyAll) {
                rowkey = Bytes.add(rowkey, Bytes.toBytes(oneKey));
            }
