...
public class CcBatchQueryImpl implements QueryBatchInterface{
        ...
        for (Result result : resultArray) {
            if (!result.isEmpty()) {
                ...
                for (String family : familyName.split(",")) {
                    ...
                    StringBuilder sb = new StringBuilder();
                    for (Cell cell : list) {
                        Risk.CallChange callChange = Risk.CallChange.parseFrom(CellUtil.cloneValue(cell));
                        if (callChange.hasIsCallChange()) sb.append(callChange.getIsCallChange());
