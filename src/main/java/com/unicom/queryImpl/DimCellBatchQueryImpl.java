...
public class DimCellBatchQueryImpl implements QueryBatchInterface{
    ...
    public HashMap<String,String>  getFieldFromHdfs(String linePath, String[] arrayField,HashMap<String,String> hdfsMap,String account) throws IOException {
        ...
        sb.append("|");
        //拼接得到k
        String lac = arrayField[8];
        String ci=arrayField[9];
        String k=lac+"|"+ci;
    ...
    public void getDataFromHbaseAndWrite(ZkInfo zkInfo,String tableName,String familyName,String columnName,
        ...
        while (it.hasNext()) {
            ...
            String lac = StringUtils.splitPreserveAllTokens(rowkeyFromHdfsMap, "|")[0];
            String ci= StringUtils.splitPreserveAllTokens(rowkeyFromHdfsMap, "|")[1];

            //得到k  获得主键
            byte[] hash = Bytes.toBytes((short) (lac.hashCode() & 0x7FFF));
            String singleRowkey = lac + ci;
            ...
            if (!result.isEmpty()) {
                ...
                for (String family : familyName.split(",")) {
                    ...
                    for (Cell cell : list) {
                        ...
                        Risk.DimCell dimCell = Risk.DimCell.parseFrom(CellUtil.cloneValue(cell));
                        if(dimCell.hasProvId()) sb.append(dimCell.getProvId());
                        sb.append("|");
                        if(dimCell.hasAreaId()) sb.append(dimCell.getAreaId());
                        sb.append("|");
                        if(dimCell.hasDistrictId()) sb.append(dimCell.getDistrictId());
                        sb.append("|");
                        if(dimCell.hasLat()) sb.append(dimCell.getLat());
                        sb.append("|");
                        if(dimCell.hasLon()) sb.append(dimCell.getLon());
                        sb.append("|");
                        if(dimCell.hasNetType()) sb.append(dimCell.getNetType());
                        sb.append("|");
                        if(dimCell.hasProvDesc()) sb.append(dimCell.hasProvDesc());
                        sb.append("|");
                        if(dimCell.hasAreaDesc()) sb.append(dimCell.getAreaDesc());
                        sb.append("|");
                        if(dimCell.hasDistrictDesc()) sb.append(dimCell.getDistrictDesc());
