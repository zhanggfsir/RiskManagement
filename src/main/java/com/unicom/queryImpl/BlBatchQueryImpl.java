...
public class BlBatchQueryImpl implements QueryBatchInterface{
    ...
    public void put2Hdfs(FileSystem fs, LoadColumnInfo loadColumnInfo, HashSet<String> lineSet, String tableName, String account,
        ...
        //5.去Hbase中查并写入文件
        getDataFromHbaseAndWrite(zkInfo319,tableName, familyName, columnName, hdfsMap,hiveOutputStream, errorOutputStream,successOutputStream);
        table.close();
    }
        ...
        for (Result result : resultArray) {
            if (!result.isEmpty()) {
                ...
                for (String family : familyName.split(",")) {
                    ...
                    StringBuilder sb = new StringBuilder();
                    for (Cell cell : list) {
                        Risk.Blacklist blacklist = Risk.Blacklist.parseFrom(CellUtil.cloneValue(cell));
                        if (blacklist.hasIsBlacklist()) sb.append(blacklist.getIsBlacklist());
