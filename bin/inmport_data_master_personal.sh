...
###############################################################################
# *脚本类型     --%@TYPE:           hbase
# *名称         --%@NAME:           import_data_master_personal.sh
# *功能描述     --%@COMMENT:      	个性化接口入主库
# *执行周期     --%@PERIOD:         D/M
...
# *目标表       --%@TO:                  
###############################################################################
# 调用方法: bash import_data_master_personal.sh day_id 20161230  dxm
###############################################################################
# 函数引用，使用相对路径
...
#创建表
java -Xmx1024m -cp ../jar/HBase_Tool.jar createTable.CreateTable ../config/hbase/master/createTable/${v_create_xml} ${v_tabname}
...
###HBase入库###
##主库##
java -Xmx4096m -cp ../jar/HBase_Tool.jar insertTable.InsertTableFromHdfs ../config/hbase/master/insertTable/${v_insert_xml} ${v_interface_name} ${v_tabname} ${v_thread_number} 2>&1 |tee $v_logfile".log" >>/dev/null
