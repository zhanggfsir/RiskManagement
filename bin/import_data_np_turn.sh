...
###############################################################################
# *脚本类型     --%@TYPE:           hbase
# *名称         --%@NAME:           import_data_master.sh
# *功能描述     --%@COMMENT:      	HBase入主库
# *执行周期     --%@PERIOD:         D
...
###############################################################################
# 调用方法: bash import_data_master.sh day_id 20161230  220120430 ant_serv
###############################################################################
# 函数引用，使用相对路径
...
#echo "v_type="$v_type
#echo "monthId="$monthId
#echo "dayId="$dayId
#echo "v_table_name="$v_table_name
#echo "v_schema_name="$v_schema_name
#echo "v_proc_name="$v_proc_name
#echo "v_nextval="$v_nextval
#echo "v_serv_code="$v_serv_code 
...
###获取文件名###
if [ $v_table_name == "dwa_v_d_cus_np_turn_info" ];then
	v_interface_name=/user/lf_by_pro/zba_dwa.db/${v_table_name}/part_id=${monthId}/day_id=${dayId}
else
...
elif [ $v_table_name == "dwa_v_d_cus_np_turn_info" ];then
  v_tabname=$v_tar_hbase_tablename
else
  ##本期HBase实体表名##
...
if [ "$v_serv_cycle" = 'D' -a "$v_tar_hbase_cycle" = 'M' -a "$dayId" = '01' ]; then
    ##主库##
	java -Xmx1024m -cp ../jar/HBase_Tool.jar createTable.CreateTable ../config/hbase/master/createTable/${v_hbase_create_xml} ${v_tabname}
fi
...
###HBase入库###
##主库##
java -Xmx4096m -cp ../jar/HBase_Tool.jar insertTable.InsertTableFromHdfs ../config/hbase/master/insertTable/${v_hbase_insert_xml} ${v_interface_name} ${v_tabname} ${v_thread_imphbase}  2>&1 |tee $v_logfile".log" >>/dev/null
#masterStatus=$?
#echo $masterStatus
...
##备库##
java -Xmx4096m -cp ../jar/HBase_Tool.jar insertTable.InsertTableFromHdfs ../config/hbase/slaver/insertTable/${v_hbasebak_insert_xml} ${v_interface_name} ${v_tabname} ${v_thread_imphbase}  2>&1 |tee $v_logfile".log" >>/dev/null
#slaverStatus=$?
#echo $slaverStatus
execlog_name_bak=$v_logfile'_bak.log'
echo $execlog_name_bak

###############################################################################
# 更新日志
# 固定模板，不需修改
#if [ $masterStatus = 0 -a $slaverStatus = 0 ];then
#select_sql="update lf_zx_mysql.${cfg_account} set update_time=now() where lower(table_name)=lower('${v_tar_hbase_tablename}');"
