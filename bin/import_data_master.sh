...
if [ "$v_tar_hbase_cycle" == "M" ];then
  ...
  v_interface_name=/user/${hbaseDB}/${database}.db/${v_table_name}/month_id=${monthId}
...
# 更新日志
# 固定模板，不需修改
#select_sql="update lf_zx_mysql.${cfg_account} set table_name='${v_tabname}',update_time=now() where lower(substr(table_name,1,length(table_name)-length('${v_condition_value}')))=lower('${v_tar_hbase_tablename}');"
