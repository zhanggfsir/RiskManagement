...
###############################################################################
# *脚本类型     --%@TYPE:           hive
# *名称         --%@NAME:           p_dm_d_cus_al_cust_info.sh
...
# *目标表       --%@TO:             ubd_risk_serv.dm_d_cus_al_cust_info
###############################################################################
# 调用方法: bash p_dm_d_cus_al_cust_info.sh yyyyMMdd
###############################################################################
set -x
##函数引用
. ./p_pub_func_all.sh
...
#例如：调用方法：./p_dm_d_cus_al_cust_info.sh 20190605 
##***************************************************************************
##加工脚本输入参数
v_date=$1
v_prov=099
...
v_procname=p_dm_d_cus_al_cust_info
v_tablename=dm_d_cus_al_cust_info
...
v_cnt1=$(is_depend_success_with_no_prov $v_date dwa_v_d_cus_al_user_info)
v_cnt2=$(is_depend_success_with_no_prov $v_date dwa_v_d_cus_al_cust_info)
echo "v_cnt1="$v_cnt1
echo "v_cnt2="$v_cnt2
...
v_sql="alter table dm_d_cus_al_cust_info drop partition (month_id='"$v_month"',day_id='"$v_day"');"
hive -e "
use $database;
$v_sql
;" 
...
#定义sql字符?
v_sql="insert overwrite table dm_d_cus_al_cust_info partition(month_id = '"$v_month"', day_id = '"$v_day"',prov_id)
select 
...
t1.prov_id
from 
(select upper(md5(device_number)) as device_number_md5,prov_id,cust_id   
  from ubd_dm.dwa_v_d_cus_al_user_info 
  where part_id='$v_month' and day_id='$v_day' ) as t1
left join 
  (select distinct cert_type,
  cust_sex,
  cert_age,
  constellation_desc,
  cust_birthday,
...
where part_id='$v_month'  and day_id='$v_day' 
) as t2
...
hive -e "
use $database;
set mapred.job.name=risk_control@p_dm_d_cus_al_cust_info_$v_month_$v_day;
...
#删除7天前的数据 
v_sql="alter table dm_d_cus_al_cust_info drop partition (month_id='"$v_7month"',day_id='"$v_7day"');"
hive -e "
use $database;
$v_sql
;"
