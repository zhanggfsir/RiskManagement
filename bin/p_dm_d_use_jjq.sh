...
###############################################################################
# *脚本类型     --%@TYPE:           hive
# *名称         --%@NAME:           p_dm_d_use_jjq.sh
# *功能描述     --%@COMMENT:      	交际圈
# *执行周期     --%@PERIOD:         D
# *参数         --%@PARAM:          帐期 YYYYMMDD
# *创建人       --%@CREATOR:
# *创建时间     --%@CREATED_TIME:   2019-06-21
# *层次         --%@LEVEL:          UBD_SERV
# *数据域       --%@DOMAIN:         RISK域
# *备注         --%@REMARK:
# *修改记录     --%@MODIFY:       	
...
# *目标表       --%@TO:             ubd_risk_serv.dm_d_use_jjq
###############################################################################
# 调用方法: bash p_dm_d_use_jjq.sh yyyyMMdd 011
###############################################################################
set -x
##函数引用
. ./p_pub_func_all.sh
...
#例如：调用方法：./p_dm_d_use_jjq.sh 20190605  011
##***************************************************************************
##加工脚本输入参数
v_date=$1
v_prov=$2
...
v_procname=p_dm_d_use_jjq
v_tablename=dm_d_use_jjq
...
#前置条件判断
v_cnt1=$(data_depend_table $v_date $v_prov dwd_d_use_mb_voice)
v_cnt2=$(data_depend_table $v_date $v_prov dwd_d_use_cb_voice)
#========================================================================
#if [ 31 -eq 31 ]; then
#if [ $v_cnt1 -eq 1 -a $v_cnt2 -eq 1 ]; then
if [ $v_cnt2 -eq 1 ]; then

v_sql="alter table dm_d_use_jjq drop partition (month_id='"$v_month"',day_id='"$v_day"',prov_id='"$v_prov"');"
hive -e "
use $database;
$v_sql
;" 
...
#定义sql字符?
v_sql="insert overwrite table dm_d_use_jjq 
partition(month_id = '"$v_month"', day_id = '"$v_day"',prov_id='"$v_prov"')
select  
upper(md5(device_number))device_number_md5,
upper(md5(oppose_number))oppose_number_md5, 
concat_ws('$',collect_list(detail)) as datail
from (
      select start_time,device_number,oppose_number,
       prov_id,concat_ws(',',start_time,end_time,cast(call_duration as string),
	   call_type,roam_type,long_type,cast(base_times as string)) as detail 
	   from ubd_dm.dwd_d_use_mb_voice where  
        part_id='$v_month' and day_id ='$v_day' 
		and prov_id='$v_prov' order by start_time desc) t1  
		group by device_number,oppose_number,prov_id
union all 
select  
upper(md5(device_number))device_number_md5,
upper(md5(oppose_number))oppose_number_md5,
...
from (
	  ...
	  prov_id,concat_ws(',',start_time,end_time,cast(call_duration as string),
      call_type,roam_type,long_type,cast(base_times as string)) as detail 
	  from ubd_dm.dwd_d_use_cb_voice where 
      part_id='$v_month' and day_id ='$v_day' 
      and prov_id='$v_prov' order by start_time desc) t2  
	  group by device_number,oppose_number,prov_id"

#hive执行sql命令，并将执行结果写入日志文件中
hive -e "
use $database;
set mapred.job.name=risk_control@p_dm_d_use_jjq_$v_month_$v_day_$v_prov;
...
#删除7天前的数据 
v_sql="alter table dm_d_use_jjq drop partition (month_id='"$v_7month"',day_id='"$v_7day"',prov_id='"$v_prov"');"
hive -e "
use $database;
$v_sql
;"
